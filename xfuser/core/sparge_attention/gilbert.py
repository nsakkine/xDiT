...
def curve(depth: int, height: int, width: int, device: torch.device) -> list[int]:
    inverse_order = []
    for i, (x, y, z )in enumerate(gilbert3d(width, height, depth)):
        inverse_order.append(z * height * width + y * width + x)
    inverse_order = torch.tensor(inverse_order, device=device)
